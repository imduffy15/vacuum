...
while :; do
    ...
    if [ -f "/root/bin/busybox" ]; then
        /root/bin/busybox ionice -c3 nice -n 10 /usr/local/bin/valetudo >> /var/log/upstart/valetudo.log 2>&1
    else
        nice -n 10 /usr/local/bin/valetudo >> /var/log/upstart/valetudo.log 2>&1
    fi
done
